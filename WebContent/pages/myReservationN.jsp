...
<style>
	body{    
        background-image: url(resources/images/background/grape.jpeg);
...
<body style="background-color: #FFFCEC;">
	...
	<div align="center" style="color:white; background-color: #263238; opacity:0.95; width: 500px; border-radius: 10px; margin: 0 auto; padding-top: 2rem; padding-bottom: 1rem;">
		<form action="turntoChatroomN.controller" method="post">
			<table>
				<tr>
					<td>用戶名：</td>
				...
				<tr>
					<td><input type="hidden" name="memId"
						value="<%=session.getAttribute("nutr.memberId")%>"></td>
					<td><input type="hidden" name="nutriId"
						value="<%=session.getAttribute("nutr.nutriId")%>"></td>
						<td><input type="hidden" name="nutrichatroomid"
						value="<%=session.getAttribute("nutri.chatroomid")%>"></td>
				</tr>
			</table>
		</form>
	</div>
